...
#Create a directory if it does not exist at present workplace
def crtdir(work_station, filename):
    if not os.path.exists(work_station + '/' + filename):
        cmd = 'mkdir ' + work_station + '/' + filename
        os.system(cmd)

#Calculate the cosine similarity between two composition vectors
def cosine_dissimilariry(pairsVector):
    sp1, sp2 = pairsVector[0], pairsVector[1]
...
#Calculate the euclidean distance between two composition vector
def euclidean_distance(pairsVector):
    sp1, sp2 = pairsVector[0], pairsVector[1]
    dict1, dict2, index1, index2 = {}, {}, set(), set()
...
cvtxt.directory\t-str\tThe path of cv.txt of species stored\n\
